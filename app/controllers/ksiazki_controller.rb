class KsiazkiController < ApplicationController
...
#4 to muzyka
    @ksiazki = Event.all(:conditions => {:category_id => 6, :magazine_id => @magazine})
  end
...
@soons = Soon.find(:all, :conditions => {:magazine_id => @magazine.id}, :order=> :position)
    ...
    @ksiazki = Event.find(params[:id])
    respond_to do |format|
      format.html  # show.html.erb
      format.xml  { render :xml => @ksiazki }
    end
  end
